...
= Configuration properties for Serverless Logic

The following table gives a reference for commonly used configuration properties supported in Serverless Logic. You can define the following properties in the `src/main/resources/application.properties` file of your project.
...
`jdbc`, `mongodb`, `filesystem`, `kafka`, `infinispan`, `postgresql`
|string
|-
|Yes
...
|Defines strategy to resolve a process version to use. The possible values of this property include `workflow`, `project`
|string
|`workflow`
|No
...
|Defines strategy to generate the configuration key of open API specifications. The possible values of this property include `file_name`, `full_uri`, `spec_title`, `function_name`
|string
|`file_name`
|No
