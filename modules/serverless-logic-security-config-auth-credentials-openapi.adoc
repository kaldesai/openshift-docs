...
. Apply MicroProfile environment variable mapping rules and replace any non-alphabetic characters with underscores `_`.

. Set the credentials by adding the configuration to your `application.properties`, workflow `ConfigMap`, or use environment variables as follows: 
+
[source,text]
----
